...
```{r}
pass = student %>% filter(UoS.Mark >= 50) %>% filter(Mode.of.Study != 'Unknown')
fail = student %>% filter(UoS.Mark < 50)

table(pass$Mode.of.Study)
table(fail$Mode.of.Study)

ggplot(student, aes(x=Mode.of.Study, y=UoS.Mark)) + geom_boxplot()

passfail <- matrix(c(9345, 245, 1182, 70), nrow = 2, ncol = 2, byrow = TRUE, dimnames = list(c("Pass", "Fail"), c("Domestic", "International")))
